...
float DallasTemperature::getTemperature() {
   if (!_addressIsCached) {
      if (getAddress(_cachedAddress, 0)) {
        _addressIsCached = true;
      } 
      else {
        return DEVICE_DISCONNECTED_C;
      }  
   ...
   if(_dataIsReady) {
      float freshTemp = getTempC(_cachedAddress);
      if (freshTemp > -50.0) {
        _lastCalculatedValue = freshTemp;
      }
      _dataIsReady = false;
   }
...
void DallasTemperature::update() {
    static bool firstRun = true;
    static unsigned long tempTimer = 0; 

    if (firstRun)                          // false after the very first call
    {
    ...
    if (millis() - tempTimer >= 750U)      // false ~99% of iterations
    {
        requestTemperatures();
        tempTimer = millis();
        _dataIsReady = true;
